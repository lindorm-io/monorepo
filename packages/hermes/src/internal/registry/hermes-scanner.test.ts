...
describe("scanModules", () => {
  ...
  beforeAll(() => {
    result = scanModules(ALL_CONSTRUCTORS);
  });
  ...
  test("should resolve @RequireNotCreated on saga create event handler", () => {
    const saga = result.sagas[0];
    const createHandler = saga.eventHandlers.find((h) => h.trigger === TestEventCreate)!;
    expect(createHandler.conditions).toMatchSnapshot();
  });
  ...
  test("should throw when view entity does not extend HermesViewEntity", () => {
    ...
    expect(() => scanModules([BadView])).toThrow(
  ...
  test("should accept Constructor[] input without file paths", () => {
    const partial = scanModules([TestCommandCreate, TestEventCreate]);
  ...
  test("should skip classes without hermes metadata", () => {
    ...
    const partial = scanModules([PlainClass, TestCommandCreate]);
    expect(partial.commands).toHaveLength(1);
  ...
  test("should silently skip multiple classes without hermes metadata", () => {
    ...
    const partial = scanModules([PlainA, PlainB, PlainC, TestEventCreate]);
  ...
  test("should throw when saga references a class without @Aggregate metadata", () => {
    ...
    expect(() => scanModules([BadSaga])).toThrow(/has no @Aggregate\(\) metadata/);
  });
});
