...
const tests: [string, Result, CaseOptions?][] = [
  [
    "",
    {
      camelCase: "",
...
describe("change case", () => {
  for (const [input, result, options] of tests) {
    test(input, () => {
      ...
      expect(headerCase(input, options)).toEqual(result.trainCase);
      expect(kebabCase(input, options)).toEqual(result.kebabCase);
      expect(lowerCase(input, options)).toEqual(result.noCase);
  ...
  describe("split", () => {
    test("should split an empty string", () => {
