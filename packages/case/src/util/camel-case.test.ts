...
describe("camelCase", () => {
  test("should convert camel case string", () => {
    expect(camelCase(inputCamel)).toMatchSnapshot();
  });

  test("should convert kebab case string", () => {
    expect(camelCase(inputKebab)).toMatchSnapshot();
  });

  test("should convert pascal case string", () => {
    expect(camelCase(inputPascal)).toMatchSnapshot();
  });

  test("should convert snake case string", () => {
    expect(camelCase(inputSnake)).toMatchSnapshot();
  });

  test("should convert array values", () => {
    expect(camelCase(inputArray)).toMatchSnapshot();
  });

  test("should convert object keys to camelCase", () => {
    expect(camelCase(inputObject)).toMatchSnapshot();
  });

  test("should throw on invalid type", () => {
    const number: any = 1234;
    expect(() => camelCase(number)).toThrow(Error);

    const error: any = new Error("error");
    expect(() => camelCase(error)).toThrow(Error);

    const date: any = new Date();
    expect(() => camelCase(date)).toThrow(Error);
  });
});
