...
describe("camelCase", () => {
  test("should convert camel case string", () => {
    expect(constantCase(inputCamel)).toMatchSnapshot();
  });

  test("should convert param case string", () => {
    expect(constantCase(inputParam)).toMatchSnapshot();
  });

  test("should convert pascal case string", () => {
    expect(constantCase(inputPascal)).toMatchSnapshot();
  });

  test("should convert snake case string", () => {
    expect(constantCase(inputSnake)).toMatchSnapshot();
  });

  test("should convert array values", () => {
    expect(constantCase(inputArray)).toMatchSnapshot();
  });

  test("should convert object keys to camelCase", () => {
    expect(constantCase(inputObject)).toMatchSnapshot();
  });

  test("should throw on invalid type", () => {
    const number: any = 1234;
    expect(() => constantCase(number)).toThrow(Error);

    const error: any = new Error("error");
    expect(() => constantCase(error)).toThrow(Error);

    const date: any = new Date();
    expect(() => constantCase(date)).toThrow(Error);
  });
});
