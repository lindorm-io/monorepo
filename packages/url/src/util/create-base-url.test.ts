...
describe("createBaseUrl", () => {
  test("should resolve with host URL", () => {
    ...
    ).toBe("https://test.lindorm.io:4000/");
  });

  test("should resolve with base URL", () => {
    expect(createBaseUrl({ base: new URL("https://test.lindorm.io:4000") }).toString()).toBe(
      "https://test.lindorm.io:4000/",
    );
  });

  test("should resolve with host string and port", () => {
    expect(createBaseUrl({ host: "https://test.lindorm.io", port: 4000 }).toString()).toBe(
      "https://test.lindorm.io:4000/",
    );
  });
  ...
  test("should resolve with base string and port", () => {
    expect(createBaseUrl({ base: "https://test.lindorm.io", port: 4000 }).toString()).toBe(
      "https://test.lindorm.io:4000/",
    );
  });
  ...
  test("should skip port", () => {
    ...
    ).toBe("https://host.lindorm.io:4000/");
  });
