...
describe("getValidUrl", () => {
  test("should resolve for URL", () => {
    expect(getValidUrl(new URL("https://test.lindorm.io:3000/test/path")).toString()).toBe(
      "https://test.lindorm.io:3000/test/path",
    );
  });

  test("should resolve for string", () => {
    expect(getValidUrl("https://test.lindorm.io:3000/test/path").toString()).toBe(
      "https://test.lindorm.io:3000/test/path",
    );
  });

  test("should resolve for string with base url", () => {
    expect(getValidUrl("/test/path", "https://test.lindorm.io:3000").toString()).toBe(
      "https://test.lindorm.io:3000/test/path",
    );
  });
