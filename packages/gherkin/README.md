...
Every error carries a `urn:lindorm:gherkin:error:<code>` type. Gherkin syntax errors, empty scenarios and zero-row `Examples:` tables are authoring errors and fail red at the offending line.
...
Shipped: everything above. Planned, not yet shipped: hooks (`@BeforeScenario`, …), shared context classes (`@Context`/`@Inject`), DataTable/DocString arguments, and tag selection. A step that carries a DataTable or DocString today fails red (`step_argument_unsupported`) rather than silently dropping the data.
