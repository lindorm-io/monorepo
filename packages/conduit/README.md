...
| ------------------ | -------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `adapter`          | `"http" \| "fetch"`              | `"http"`        | Axios adapter. `"http"` uses Node `http`/`https`; `"fetch"` uses native fetch / undici. A per-request `adapter` overrides it.                                                                                                                                                                                                                               |
...
| `config`           | `RawAxiosRequestConfig` (subset) | `{}`            | Native Axios config pass-through. Every field Conduit owns is removed from the type, so it cannot be set here: `adapter`, `auth`, `baseURL`, `data`, `headers`, `method`, `params`, `paramsSerializer`, `responseType`, `timeout`, `transformRequest`, `transformResponse`, `url`, `validateStatus`, `withCredentials`. Use the first-class option instead. |
| `environment`      | `Environment`                    | `null`          | Sent as the `X-Environment` request header.                                                                                                                                                                                                                                                                                                                 |
| `expectedResponse` | `ExpectedResponse`               | `"json"`        | How every response body is parsed, forwarded to the Axios `responseType`. A per-request `expectedResponse` overrides it. `"arraybuffer"` yields a Node `Buffer`.                                                                                                                                                                                            |
...
| `lookup`           | `ConduitLookup`                  | `undefined`     | DNS resolver hook forwarded to the `http` adapter (SSRF IP-pinning): resolve + validate once, return the vetted `{ address, family }`, and the socket connects to that IP while `Host`/TLS SNI keep the hostname. A per-request `lookup` overrides it. Node-only; the `fetch` adapter ignores it.                                                           |
...
| `timeout`          | `number`                         | `30000`         | Request timeout in milliseconds, applied to every request. A per-request `timeout` overrides it.                                                                                                                                                                                                                                                            |
| `withCredentials`  | `boolean`                        | `undefined`     | Whether to send credentials with cross-origin requests. A per-request `withCredentials` overrides it.                                                                                                                                                                                                                                                       |
...
`expectedResponse` accepts `"arraybuffer" | "blob" | "document" | "formdata" | "json" | "stream" | "text"`. It overrides the constructor-level `expectedResponse`; the same holds for `adapter`, `lookup`, `timeout` and `withCredentials`. None of these can be set through `config` — they are removed from its type.
