...
describe("resolveMiddleware", () => {
  ...
  test("should resolve", async () => {
    await expect(resolveMiddleware(ctx, [mw])).resolves.toStrictEqual({
      after: 1,
      before: 1,
      context: "context",
      mwafter: true,
    ...
    expect(ctx).toStrictEqual({ context: "context" });
  });

  test("should resolve without cloning context", async () => {
    await expect(resolveMiddleware(ctx, [mw], { useClone: false })).resolves.toStrictEqual({
      after: 1,
      before: 1,
      context: "context",
      mwafter: true,
