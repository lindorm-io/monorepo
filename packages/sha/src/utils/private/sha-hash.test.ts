...
describe("sha-hash", () => {
  describe("SHA256", () => {
    test("should create hash at base64 digest", () => {
      expect(createShaHash({ algorithm: "SHA256", data: "data", format: "base64" })).toBe(
        "Om6weQ85rIfJTzhWst0sXREOaBFgImGpqSPTuyOtyLc=",
      );
    });

    test("should create hash at hex digest", () => {
      expect(createShaHash({ algorithm: "SHA256", data: "data", format: "hex" })).toBe(
  ...
  describe("SHA384", () => {
    test("should create hash at base64 digest", () => {
      expect(createShaHash({ algorithm: "SHA384", data: "data", format: "base64" })).toBe(
        "IDng8LknKEmfuI4j68PP0FVLKEALDte3UwVciLWGXDwqpyxqGprgp1XYeQCkpv9B",
      );
    });

    test("should create hash at hex digest", () => {
      expect(createShaHash({ algorithm: "SHA384", data: "data", format: "hex" })).toBe(
  ...
  describe("SHA512", () => {
    test("should create hash at base64 digest", () => {
      expect(createShaHash({ algorithm: "SHA512", data: "data", format: "base64" })).toBe(
        "d8fOml2GuzhtRDu5Y5D6oSBjMVhpnIhEwwsTqwv5J2C35EFq6jl9uRtKwOXdVrjvfksGYWKrH9wIgxnObe/Idg==",
      );
    });

    test("should create hash at hex digest", () => {
      expect(createShaHash({ algorithm: "SHA512", data: "data", format: "hex" })).toBe(
