...
/**
 * The boundary, pinned with LITERALS. Both IANA registries put it at the same
 * integer and state it in the same words — RFC 8152 §16.2 (COSE Header
 * Parameters) and RFC 8392 §9.1.1 (CWT Claim Key): *"Integer values less than
 * -65536 are marked as private use."*
 *
 * ⚠ `-65536` ITSELF IS NOT PRIVATE USE. It is the last value of the range above,
 * and an off-by-one here goes the dangerous way: it would degrade a parameter
 * that is registered elsewhere, silently renaming somebody else's label.
 */
describe("isPrivateUseLabel", () => {
  ...
  test.each([
    // RFC 9052 §3.1 Table 2 — the core COSE header parameters.
    1, 2, 3, 4, 5, 16,
    // RFC 9360 §2 — the X.509 parameters.
    33, 34, 35,
    // RFC 8392 §4 Table 1 — the registered CWT claim keys.
    7,
