 ...
 * reader choose: `placement` (`is-protected-only.ts`) is the same datum the WRITE
 * side enforces, so the only parameters that can enter the domain header from the
 * unauthenticated bucket are the two the registry marks `"either"` — `kid` and
 ...
 * DROPPED here, silently and unconditionally, exactly as it is REFUSED on write.
 *
 ...
 */
export const mergeHeaderBuckets = (buckets: WireHeaderBuckets): WireTokenHeader => {
  const merged: Dict = {};
