...
export const SCENARIOS: ReadonlyArray<Scenario> = [
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    knownDefect:
      "translate.ts wireToDomain and extract-claims.ts prefer the DOMAIN name over the wire name, on the raw-token path as well as the loose door, so a custom `audience` claim shadows the wire `aud` before the floor ever reads it.",
    given: [
      {
        step: "token",
        via: "kit-sign",
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    then: [{ step: "rejects", error: "AegisDomainError" }],
    absentTwin: {
      jose: "COSE-ONLY. extractDomainClaims keeps an unregistered key VERBATIM, so a JOSE `expires_at` never becomes `expiresAt` and the JOSE floor already refuses — a JOSE twin would assert a rule that is not at risk on that wire",
    },
  },
  {
