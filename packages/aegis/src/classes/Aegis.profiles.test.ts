...
describe("Aegis profiles", () => {
  ...
  describe("access_token", () => {
    ...
    test("refuses to mint when the vault holds no asymmetric signing key", async () => {
      ...
      const symAmphora = new Amphora({ internal: { issuer: ISSUER }, logger: symLogger });
  ...
  describe("id_token", () => {
    ...
    test("EdDSA id token produces a SHA-512 256-bit at_hash", async () => {
      const okpAmphora = new Amphora({ internal: { issuer: ISSUER }, logger });
    ...
    test("ML-DSA-65 id token produces a SHA-512 256-bit at_hash", async () => {
      const akpAmphora = new Amphora({ internal: { issuer: ISSUER }, logger });
    ...
    test("a *256 signing alg (ES256) produces a SHA-256 128-bit at_hash", async () => {
      ...
      const es256Amphora = new Amphora({ internal: { issuer: ISSUER }, logger });
  ...
  describe("ML-DSA end-to-end (RFC 9964)", () => {
    ...
    beforeEach(async () => {
      akpAmphora = new Amphora({ internal: { issuer: ISSUER }, logger });
      await akpAmphora.setup();
      akpAmphora.add(TEST_AKP_KEY_SIG);
