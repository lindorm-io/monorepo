...
/**
 * Drift guard (b): `CwtClaimsWire`' omit/add transformation must equal the
 ...
 * (below), and the type-level checks force `CwtClaimsWire` to actually carry the
...
// the actual `CwtClaimsWire` / `JwtClaimsWire` member types.
...
//   - the JOSE name is a real (typed) member of `JwtClaimsWire`,
//   - the JOSE name is NOT a real member of `CwtClaimsWire` (dropped),
//   - the COSE name IS a real member of `CwtClaimsWire` (added),
//   - the COSE name is NOT a real member of `JwtClaimsWire`.
...
type DivergenceDrift = {
  [Jose in keyof Divergences]: IsReal<JwtClaimsWire[Jose]> extends true
    ? IsReal<CwtClaimsWire[Jose]> extends false
      ? IsReal<CwtClaimsWire[Divergences[Jose]]> extends true
        ? IsReal<JwtClaimsWire[Divergences[Jose]]> extends false
          ? never
          : Divergences[Jose] // cose name leaked into JwtClaimsWire
        : Divergences[Jose] // cose name missing from CwtClaimsWire
      : Jose // jose name not dropped from CwtClaimsWire
    : Jose; // jose name missing from JwtClaimsWire
}[keyof Divergences];
