...
export const writeSource = async (options: WriteSourceOptions): Promise<void> => {
  ...
  const isSql = PROTEUS_SQL_DRIVERS.includes(driver);
  ...
  if (isSql) {
    files.push({
      path: join(resolvedDirectory, "migrations", ".gitkeep"),
      content: "",
    });
  }
  ...
  Logger.std.log(`  ${join(resolvedDirectory, "entities/")}`);

  if (isSql) {
    Logger.std.log(`  ${join(resolvedDirectory, "migrations/")}`);
  }
};
