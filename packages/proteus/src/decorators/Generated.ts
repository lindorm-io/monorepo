 ...
 * - `@Generated("increment")` — portable, overridable DB auto-increment (postgres
 *   `GENERATED BY DEFAULT AS IDENTITY`, mysql `AUTO_INCREMENT`, sqlite `AUTOINCREMENT`)
 * - `@Generated("identity")` — postgres-only strict `GENERATED ALWAYS AS IDENTITY`
 *   (the DB owns the value; explicit inserts are rejected); THROWS on mysql/sqlite,
 *   which have no strict identity mode — use `increment` for portable auto-increment
