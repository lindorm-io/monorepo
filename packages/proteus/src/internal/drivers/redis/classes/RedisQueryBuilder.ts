...
export class RedisQueryBuilder<E extends IEntity> extends QueryBuilder<E> {
  ...
  override include(relation: string): this {
    throw new NotSupportedError("include is not supported by the Redis driver", {
      ...
      title: "Unsupported Operation",
      details:
        "Query-builder relation loading is implemented by the postgres, mysql and sqlite drivers only — this is a driver gap, not a usage error. On the Redis driver, load relations through the repository instead: repository.find(criteria, { relations: [...] }).",
      data: {
        operation: "include",
        relation,
