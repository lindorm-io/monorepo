...
> implements IUpdateQueryBuilder<E> {
  ...
  async execute(): Promise<WriteResult<E>> {
    ...
    const pushSet = (column: string, value: unknown): void => {
      params.push(value);
      setClauses.push(`${quoteIdentifier(column)} = $${params.length}`);
    };
    ...
    for (const [key, value] of Object.entries(this.data)) {
      ...
      if (field?.typedJson) {
        ...
        pushSet(field.name, coerceWriteValue(data, field ?? null));
    ...
    let text = `UPDATE ${tableName} AS "t0" SET ${setClauses.join(", ")} ${withPredicates(whereClause, discPredicate)}`;
