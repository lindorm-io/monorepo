 ...
 * string name (skips inverse-side `joinTable: true`). The composite PK covers the
 * owner-side FK; a secondary index is created for foreign-side FK columns.
 */
export const generateJoinTableDDL = (
  ...
  for (const relation of metadata.relations) {
    ...
    for (const [joinCol, ownerPk] of Object.entries(relation.joinKeys)) {
      ...
      lines.push(
        `${quoteIdentifier(joinCol)} ${colType} NOT NULL` +
          ` REFERENCES ${qualifiedOwner} (${quoteIdentifier(resolvedOwnerPk)})` +
    ...
    for (const [joinCol, foreignPk] of Object.entries(inverseRelation.joinKeys)) {
      ...
      lines.push(
        `${quoteIdentifier(joinCol)} ${colType} NOT NULL` +
          ` REFERENCES ${qualifiedForeign} (${quoteIdentifier(resolvedForeignPk)})` +
          ` ON DELETE CASCADE ON UPDATE CASCADE`,
      );
      pkCols.push(quoteIdentifier(joinCol));
    ...
    tables.push(
      `CREATE TABLE IF NOT EXISTS ${qualifiedJoinTable} (\n${body},\n  PRIMARY KEY (${pkCols.join(", ")})\n);`,
    );
    ...
    if (foreignSideCols.length > 0) {
      ...
      indexes.push(
        `CREATE INDEX IF NOT EXISTS ${quoteIdentifier(indexName)} ON ${qualifiedJoinTable} (${indexCols});`,
      );
    }
  }
