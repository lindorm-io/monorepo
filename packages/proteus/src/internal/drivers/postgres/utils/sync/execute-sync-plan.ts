...
export class SyncPlanExecutor {
  ...
  execute = async (
    ...
    const result = await withAdvisoryLock(
      ...
      async () => {
        ...
        // database-wide lock in their own transaction (see createExtensionLocked).
  ...
  /**
   * Create one extension out of band — its own transaction, under the
   * database-wide lock shared with the migration path. Returns whether this
   * session did the creating; the mechanics live in `createExtensionLocked`.
   */
  private createExtension = async (
    ...
    switch (outcome.status) {
      ...
      case "failed":
        throw new PostgresSyncError(
          `Failed to create extension "${op.extension ?? "?"}"`,
          {
            code: "sync_failed",
            title: "Sync Failed",
            details:
      ...
      default:
        throw new PostgresSyncError(
          `Unhandled extension outcome: "${(outcome as CreateExtensionOutcome).status}"`,
          {
            ...
            title: "Sync Failed",
            details:
              "Creating a PostgreSQL extension returned an outcome the sync executor does not handle.",
            data: { extension: op.extension ?? null },
          },
        );
    }
