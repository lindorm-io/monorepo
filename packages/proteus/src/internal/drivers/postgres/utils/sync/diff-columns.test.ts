...
describe("diffColumns", () => {
  ...
  it("should alter identity generation from ALWAYS to BY DEFAULT (increment correctness fix)", () => {
    const ops = diffColumns(
      [
        makeDbCol({
          name: "id",
    ...
    const op = ops.find((o) => o.type === "alter_column_identity");
    expect(op).toBeDefined();
    expect(op!.sql).toContain("SET GENERATED BY DEFAULT");
  });
