 ...
 *   WITH to_delete AS (SELECT pk FROM table WHERE ... LIMIT ?)
...
export const compileDeleteWithLimit = <E extends IEntity>(
  ...
  // CTE: SELECT pk FROM table [INNER JOIN child] WHERE ... LIMIT ?
  const cteSelect = [
  ...
  const pkCondition =
    pkCols.length === 1
      ...
      : `(${pkCols.join(", ")}) IN (SELECT ${pkCols.join(", ")} FROM "to_delete")`;
