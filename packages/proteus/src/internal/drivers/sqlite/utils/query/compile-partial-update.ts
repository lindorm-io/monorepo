...
export const compilePartialUpdate = <E extends IEntity>(
  ...
  const text = `UPDATE ${tableName} SET ${setClauses.join(", ")} WHERE ${conditions.join(" AND ")} RETURNING *`;
...
export const compileJoinedPartialUpdate = <E extends IEntity>(
  ...
  if (rootSetClauses.length > 0) {
    ...
    const rootText = `UPDATE ${rootTableName} SET ${rootSetClauses.join(", ")} WHERE ${rootConditions.join(" AND ")} RETURNING *`;
  ...
  if (hasChildChanges) {
    ...
    const childText = `UPDATE ${childTableName} SET ${childSetClauses.join(", ")} WHERE ${childConditions.join(" AND ")} RETURNING *`;
  ...
  return { rootSql, childSql, rootIsUpdate: rootSetClauses.length > 0 };
};
