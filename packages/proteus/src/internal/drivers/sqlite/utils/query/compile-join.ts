...
const compileManyToManyJoin = (
  ...
  // SQLite has no schemas — just quote the table name
  const joinTableQualified = quoteIdentifier(relation.joinTable as string);
  const targetTableQualified = quoteIdentifier(foreignMeta.entity.name);
