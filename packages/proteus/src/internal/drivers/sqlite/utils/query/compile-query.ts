...
): CompiledQuery => sharedCompileQuery(state, metadata, sqliteDialect, deps, namespace);

export const compileCount = <E extends IEntity>(
  state: QueryState<E>,
  metadata: EntityMetadata,
  namespace?: string | null,
): CompiledQuery => sharedCompileCount(state, metadata, sqliteDialect, deps, namespace);
