...
describe("generateCheckDDL (MySQL)", () => {
  ...
  test("throws ProteusError when named constraint exceeds 64 characters", () => {
  ...
  test("accepts a named constraint of exactly 64 characters", () => {
  ...
  test("auto-names differ for different tables with same expression", () => {
    const checks = [check("value > 0")];
