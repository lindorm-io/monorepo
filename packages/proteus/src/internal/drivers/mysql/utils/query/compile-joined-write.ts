...
export type JoinedInsertSql = {
   ...
   * Used by the executor to patch generated PK values from the root INSERT.
   */
 ...
 *
 * MySQL has no RETURNING clause. The executor must run rootSql first,
 * then childSql, within a transaction. For AUTO_INCREMENT PKs, the executor
 * patches childSql.params with the insertId from rootSql.
 */
export const compileJoinedInsert = <E extends IEntity>(
  ...
  const rootText = `INSERT INTO ${rootTableName} (${rootColNames.join(", ")}) VALUES (${rootPlaceholders.join(", ")})`;
  ...
  // push sent a `bigint` PK to the driver as a BigInt on the child INSERT while
  // the root got the decimal string `dehydrateEntity` produced: one value, two
  // shapes, one statement.
  ...
  const childText = `INSERT INTO ${childTableName} (${childColNames.join(", ")}) VALUES (${childPlaceholders.join(", ")})`;
...
export const compileJoinedUpdate = <E extends IEntity>(
  ...
  if (rootUpdateColumns.length > 0) {
    ...
    const rootText = `UPDATE ${rootTableName} SET ${setClauses.join(", ")} WHERE ${pkConditions.join(" AND ")}`;
  ...
  if (childUpdateColumns.length > 0) {
    ...
    const childText = `UPDATE ${childTableName} SET ${setClauses.join(", ")} WHERE ${pkConditions.join(" AND ")}`;
