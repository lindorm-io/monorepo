...
export const compileRelationQuery = (
  ...
  const foreignResolved = resolveTableName(foreignMeta, ctx.namespace);
  const schema = foreignResolved.schema;
...
const buildRelationOrderBy = (
  ...
  const clauses = Object.entries(relation.orderBy).flatMap(([key, dir]) => {
    const colName = resolveColumnNameSafe(foreignMeta.fields, key);
    const qualifiedCol = `${prefix}${quoteIdentifier(colName)}`;
    const upperDir = (dir as string).toUpperCase();
