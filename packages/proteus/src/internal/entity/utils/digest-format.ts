...
/**
 * Expected stored-value format for a @Sensitive digest algorithm. Format
 * validation only — proteus never hashes or verifies; it just catches the one
 * bug that matters (a plaintext value stored in a hash column).
 */
export const digestFormatRegex = (digest: SensitiveDigest): RegExp => {
  switch (digest) {
    ...
    default:
      throw new EntityMetadataError(`Unknown sensitive digest "${digest as string}"`, {
