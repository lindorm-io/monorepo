...
- `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` — the root middleware chain every scanned route/listener inherits. Pylon installs no middleware of its own on the strength of a setting, so **mounting is the switch** for both features that appear here, and each is emitted together with the block in `pylon.ts` that is its policy:
  - With audit logging selected it mounts `useAuditLog()` **outermost** — ahead of the limiter. A middleware records only what it wraps, and the limiter rejects by throwing, so audit mounted below it would never see a 429. Denied and throttled requests are the ones an auditor wants
  - With rate limiting selected it mounts `useRateLimit()` next, and with no arguments, so it holds no second copy of the window and ceiling — which is why the two are emitted together: a bare mount with no block is bounded by nothing and throws `rate_limit_not_bounded` on every request
...
| `writeMiddlewareFiles`       | `(answers: Answers) => void`                                  | Writes the root `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` for the selected transports, mounting a bare `useAuditLog()` and then a bare `useRateLimit()` for the chosen features — each governed by the block `writePylonFile` emits alongside it. |
| `buildMiddlewareFile`        | `(answers: Answers, transport: "http" \| "socket") => string` | The content `writeMiddlewareFiles` writes, returned as a string.                                                                                                                                                                                                     |
