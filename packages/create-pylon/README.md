...
- `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` — the root middleware chain every scanned route/listener inherits. With rate limiting selected it mounts `useRateLimit()` **first**, because Pylon installs no limiter of its own: the `rateLimit` block in `pylon.ts` is policy, and this mount is what applies it. The mount takes no arguments, so it holds no second copy of the window and ceiling
...
| `writeMiddlewareFiles`       | `(answers: Answers) => void`                                  | Writes the root `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` for the selected transports, mounting `useRateLimit()` when rate limiting is chosen.                                                                                    |
