...
- `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` — the root middleware chain every scanned route/listener inherits. With rate limiting selected it mounts `useRateLimit()` **first**. Pylon installs no limiter of its own, so the mount is the switch; the `rateLimit` block in `pylon.ts` is only policy. The mount takes no arguments, so it holds no second copy of the window and ceiling — which is why the two are emitted together: a bare mount with no block is bounded by nothing and throws `rate_limit_not_bounded` on every request
...
| `writeMiddlewareFiles`       | `(answers: Answers) => void`                                  | Writes the root `src/routes/_middleware.ts` / `src/listeners/_middleware.ts` for the selected transports, mounting a bare `useRateLimit()` when rate limiting is chosen — bounded by the `rateLimit` block `writePylonFile` emits alongside it.      |
