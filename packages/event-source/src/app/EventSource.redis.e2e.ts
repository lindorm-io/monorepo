...
describe("EventSource (Redis)", () => {
  ...
  beforeAll(async () => {
    ...
    await app.setup.registerAggregateCommandHandler(
      new AggregateCommandHandlerImplementation<CreateGreeting, GreetingCreated>({
        ...
        aggregate: { name: "test_aggregate", context: "es_redis" },
    ...
    await app.setup.registerQueryHandler(
      new QueryHandlerImplementation<QueryGreeting, unknown>({
        ...
        view: { name: "test_view", context: "es_redis" },
    ...
    await app.setup.registerSagaEventHandler(
      new SagaEventHandlerImplementation<GreetingCreated>({
        eventName: "greeting_created",
        aggregate: { name: "test_aggregate", context: "es_redis" },
        saga: { name: "test_saga", context: "es_redis" },
        conditions: { created: false },
    ...
    app.setup.registerViewAdapter({ name: "test_view", context: "es_redis", type: "redis" });
  }, 30000);
  ...
  test("should publish", async () => {
    ...
    app.on("view.es_redis", onEventSpyContext);
    app.on("view.es_redis.test_view", onEventSpyName);
    app.on(`view.es_redis.test_view.${id}`, onEventSpyId);
