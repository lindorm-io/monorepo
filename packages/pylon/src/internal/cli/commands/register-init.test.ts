...
describe("registerInitCommand", () => {
  ...
  it("should wire init as the action", async () => {
    ...
    await program.parseAsync(["node", "pylon", "init"]);
  ...
  it("should pass --directory option through", async () => {
    ...
    await program.parseAsync(["node", "pylon", "init", "-d", "./custom"]);

    const [opts] = mockInit.mock.calls[0];
    expect((opts as any).directory).toBe("./custom");
  });

  it("should pass --dry-run option through", async () => {
    mockInit.mockResolvedValue(undefined);

    await program.parseAsync(["node", "pylon", "init", "--dry-run"]);
