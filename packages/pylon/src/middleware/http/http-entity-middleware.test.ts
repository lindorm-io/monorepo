...
import { createHttpEntityMiddleware } from "./http-entity-middleware";
...
describe("createHttpEntityMiddleware", () => {
  const next = jest.fn();
  ...
  test("should find entity and set on context", async () => {
    await expect(
      createHttpEntityMiddleware(TestEntityOne, source)("data.id")(ctx, next),
  ...
  test("should skip optional entity", async () => {
    ...
    await expect(
      createHttpEntityMiddleware(TestEntityOne, source)("data.id", { mandatory: false })(
        ctx,
        next,
      ),
    ).resolves.not.toThrow();
  ...
  test("should throw on mandatory entity", async () => {
    ...
    await expect(
      createHttpEntityMiddleware(TestEntityOne, source)("data.id", { mandatory: true })(
        ctx,
        next,
      ),
    ).rejects.toThrow();
