...
const app = new Pylon({
  socket: {
    ...
    connectionMiddleware: [useAccessToken({ audience: "https://api.example.com" })],
  },
  // …
});
```
...
listener — it dispatches on the context it is handed. It takes no issuer and
requires an audience (see [Authentication](#authentication)).
...
// The audience is REQUIRED: it is this service's own identifier.
const accessAuth = useAccessToken({ audience: "https://my-api.example.com" });
...
The RFC 7662 response members that describe the ANSWER rather than the token — `active` and `token_type` — are kept out of the bucket, and off the resolved credential entirely. They are still ASSERTED first. `active` is primary. `token_type` is RFC 6749 §7.1's **presentation scheme** (`Bearer`/`DPoP`), a homonym of the JOSE `typ` the local route's profile floor matches — so it is compared against the scheme the request actually used, case-insensitively (RFC 7235 §2.1), and only in the direction that carries a security claim: an answer of `DPoP` for a credential presented as a bearer token is refused (`introspection_token_type_mismatch`, 401), because the authorization server is saying the credential is proof-of-possession bound while the request presented no proof — and when the answer also omits `cnf.jkt` there is nothing left for the binding check to catch it with. The reverse needs no rule: a `Bearer` answer for a credential presented under `DPoP` is the request holding itself to the stricter standard, and the binding is enforced from `cnf.jkt` and the proof either way. **An answer that states no `token_type` at all is served** — RFC 7662 §2.2 makes every member a MAY. RFC 7662 `username` is the opposite case: an authorization server that can report one means a token can carry one, so it is a registered aegis claim and arrives at `access.claims.username` on **both** paths. (It is not OIDC's `preferred_username`, which is a profile field — see below.) And `custom` is reserved at the top level of an introspection answer: a server returning a member literally named `custom` gets it back at `access.custom.custom`, not shadowing the bucket.
...
**DPoP (RFC 9449)** is checked from `(proof, claims.confirmation.thumbprint, token)`, so it runs identically on both credential routes AND on both transports — RFC 9449 §6.2 conveys `cnf.jkt` in the introspection response precisely so a resource server can validate the binding locally for an opaque token. A token carrying `cnf.jkt` is refused without a matching proof (`missing_dpop_proof`), a token _without_ `cnf.jkt` presented under the `DPoP` scheme is refused as `token_not_dpop_bound`, and a proof that fails its own verification is `invalid_dpop_proof`. On the introspected route there is one binding an answer can assert that `cnf.jkt` cannot carry — an answer of `token_type: DPoP` with no confirmation in it — and that is caught by the `token_type` comparison above (`introspection_token_type_mismatch`). Only `htm`/`htu` are transport-specific: HTTP reads them off the request, a handshake reconstructs them from the upgrade (`GET`, the socket.io path).

⚠ **A socket handshake presents no authorization scheme at all** — socket.io carries the credential in the `auth.bearer` payload, not an `Authorization` header — so the `token_type` comparison has nothing to compare on that transport and any stated type is accepted there (the same reason `token_not_dpop_bound` cannot fire on a handshake: the mount's `dpop` mode carries the binding intent instead). The socket refresh event is the same. Deriving a pseudo-scheme from the presence of a DPoP proof header would refuse the `DPoP` answer that `dpop: "disabled"` exists to accept, so the gap is stated rather than papered over.

The COOKIE-SESSION route carries no proof and runs no binding check of its own — it does not need one. It verifies with aegis's RFC 9449-strict default, so a `cnf.jkt` token with no proof never verifies at all and is refused as `invalid_session_access_token`. The header routes need their own check precisely because they tell aegis to TRUST the binding (`trustBoundThumbprint`) in order to own the proof comparison themselves — verifying one proof once, on both credential routes, rather than twice on one and once on the other.
...
- **`publish: false`** keys stay in the vault for internal use (cookie/session crypto) and are **excluded from the JWKS** — and, since Amphora's `find`/`filter` default to `{ publish: true }`, from ordinary key selection too. To select an internal key you must ask for it: `{ publish: false, … }`. The exception is the **cookie key selectors**, `signature` and `encryption` alike — a key that seals a value this server reopens, or signs a cookie only this server verifies, is unpublished by definition, so those selectors default to `publish: false` themselves. `findById` is unfiltered — an explicit kid is explicit intent.
...
- ⚠ **`publish: false` is the default on both `signature` and `encryption`** (amphora's `UNPUBLISHED_DEFAULT`). Amphora's ordinary query is the **published** set, which hides exactly the internal cookie/session key these selectors describe — a value this server seals and reopens, a signature this server alone verifies, neither of which any relying party ever sees. So `{ condition: { purpose: "cookie" } }` reaches the internal cookie key on its own, for both roles. Stating `publish: false` changes nothing; state `publish: true` to deliberately use a published key. ⚠ This is the **cookie** answer, not a general one: a **token** signature must use a published key, and `@lindorm/aegis` applies no such default.
- **Verification takes no default**, unlike signing. It resolves the key the cookie's `.kid` names through the unfiltered `findById` and then _checks_ the condition against it — there is no publish gate to reach past, so a default would only assert that the key is unpublished, which is policy the deployment never stated.
- **A cookie is signed only when a signing key is named.** There is no fallback to the floor plus default alone — it would resolve to whichever internal signing key is newest, which may be the session key or another role's entirely. So no `cookies.signature` ⇒ unsigned cookies; a per-call `{ signature: true }` with none configured throws rather than guessing. Since `auth.session` chains to `cookies`, a session cookie is signable iff a cookie signing key is named.
