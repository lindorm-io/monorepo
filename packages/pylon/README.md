...
- **`publish: false`** keys stay in the vault for internal use (cookie/session crypto) and are **excluded from the JWKS** — and, since Amphora's `find`/`filter` default to `{ publish: true }`, from ordinary key selection too. To select an internal key you must ask for it: `{ publish: false, … }`. The one exception is **encryption** — a key sealing a value this server reopens is unpublished by definition, so those selectors default to `publish: false` themselves. `findById` is unfiltered — an explicit kid is explicit intent.
...
- ⚠ **`publish: false` is load-bearing on `signature`, and the default on `encryption`.** Amphora's default query is the **published** set, so an internal cookie/session **signing** key is unreachable without it — omit it and you select the JWKS token key, or resolve nothing. **Encryption** defaults to `publish: false` (amphora's `ENVELOPE_DEFAULT`): a key that seals a value this same server reopens is unpublished by definition, so `{ condition: { purpose: "cookie" } }` reaches the internal cookie key on its own. Stating it changes nothing; state `publish: true` to deliberately seal with a published key.
