...
describe("getExpiryDate", () => {
  ...
  test("should throw when expiry is invalid type", () => {
    // @ts-ignore
    expect(() => getExpiryDate(true)).toThrow(DateError);
  });

  test("should throw when number expiry is not in seconds", () => {
    expect(() => getExpiryDate(10000000000)).toThrow(DateError);
  });

  test("should throw when expiry is before current date", () => {
    expect(() => getExpiryDate(new Date("1999-01-01T08:00:00.000Z"))).toThrow(DateError);
  });
});
