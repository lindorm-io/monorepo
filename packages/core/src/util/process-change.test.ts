...
describe("objectDifference", () => {
  ...
  test("should resolve changes to objects", () => {
    ...
    expect(test.state).toStrictEqual({ one: { moon: 1 } });
    expect(test.meta).toStrictEqual({
      one: {
        moon: {
          destroyed: false,
  ...
  test("should resolve changes to array with primitives", () => {
    ...
    expect(test.state).toStrictEqual({ one: [1, 2, 3] });
    expect(test.meta).toStrictEqual({
      one: [
    ...
    expect(test.state).toStrictEqual({ one: [1, 2, 3, 4] });
    expect(test.meta).toStrictEqual({
      one: [
        { destroyed: false, value: 1, timestamp: ts1 },
        { destroyed: false, value: 2, timestamp: ts1 },
    ...
    expect(test.state).toStrictEqual({ one: [3, 4] });
    expect(test.meta).toStrictEqual({
      one: [
        {
          destroyed: true,
  ...
  test("should resolve changes to array with objects", () => {
    ...
    expect(test.state).toStrictEqual({ one: [{ key: 1, number: 1 }] });
    expect(test.meta).toStrictEqual({
      one: [
        {
          destroyed: false,
