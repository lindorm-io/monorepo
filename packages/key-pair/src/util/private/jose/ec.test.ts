...
describe("ec", () => {
  ...
  test("should encode both keys", () => {
    expect(encodeEC({ privateKey, publicKey, crv })).toStrictEqual({ crv, d, x, y });
  });

  test("should encode public key", () => {
    expect(encodeEC({ publicKey, crv })).toStrictEqual({ crv, x, y });
  });

  test("should decode both keys", () => {
    expect(decodeEC({ d, x, y, crv })).toStrictEqual({ publicKey, privateKey });
  });

  test("should decode public key", () => {
    expect(decodeEC({ x, y, crv })).toStrictEqual({ publicKey });
  });

  test("should resolve a valid public key", () => {
    const { publicKey: decodedPublicKey } = decodeEC({ x, y, crv });
  ...
  test("should resolve a valid private key", () => {
    const { privateKey: decodedPrivateKey } = decodeEC({ d, x, y, crv });
