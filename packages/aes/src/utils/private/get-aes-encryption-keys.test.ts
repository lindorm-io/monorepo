import { privateEncrypt as _privateEncrypt, publicEncrypt as _publicEncrypt } from "crypto";
...
describe("getAesEncryptionKeys", () => {
  beforeEach(() => {
    ...
    privateEncrypt.mockReturnValue(Buffer.from("private-encryption-key"));
    publicEncrypt.mockReturnValue(Buffer.from("public-encryption-key"));
  });

  afterEach(jest.clearAllMocks);

  test("should resolve encryption keys with private key", () => {
    expect(
      ...
      publicEncryptionKey: Buffer.from("private-encryption-key"),
    });

    expect(privateEncrypt).toHaveBeenCalled();
  });

  test("should resolve encryption keys with public key", () => {
    expect(
      getAesEncryptionKeys({
        algorithm: AesAlgorithm.AES_256_GCM,
        key: PUBLIC_KEY,
  ...
  test("should resolve encryption keys with secret", () => {
    ...
    expect(privateEncrypt).not.toHaveBeenCalled();
    expect(publicEncrypt).not.toHaveBeenCalled();
  });
