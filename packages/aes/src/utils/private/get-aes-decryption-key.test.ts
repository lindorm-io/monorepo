import { privateDecrypt as _privateDecrypt, publicDecrypt as _publicDecrypt } from "crypto";
...
describe("getAesDecryptionKey", () => {
  beforeEach(() => {
    ...
    privateDecrypt.mockReturnValue(Buffer.from("private-encryption-key"));
    publicDecrypt.mockReturnValue(Buffer.from("public-encryption-key"));
  });

  afterEach(jest.clearAllMocks);

  test("should resolve decryption key with private key", () => {
    expect(
      getAesDecryptionKey({
    ...
    ).toStrictEqual(Buffer.from("private-encryption-key"));

    expect(privateDecrypt).toHaveBeenCalled();
  });

  test("should resolve decryption key with public key", () => {
    expect(
      getAesDecryptionKey({
        algorithm: AesAlgorithm.AES_256_GCM,
        key: PUBLIC_KEY,
    ...
    ).toStrictEqual(Buffer.from("public-encryption-key"));
  ...
  test("should resolve decryption key with secret", () => {
    ...
    expect(privateDecrypt).not.toHaveBeenCalled();
  ...
  test("should throw error when public encryption key is missing", () => {
    expect(() =>
      getAesDecryptionKey({
        algorithm: AesAlgorithm.AES_256_GCM,
        key: PUBLIC_KEY,
