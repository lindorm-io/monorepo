...
describe("is-aes", () => {
  ...
  describe("isAesString", () => {
    test("should return true for string starting with 'aes:'", () => {
      expect(isAesString("aes:header$iv$tag$ciphertext")).toBe(true);
    });

    test("should return true for any string starting with 'aes:'", () => {
      expect(isAesString("aes:something")).toBe(true);
    });

    test("should return false when string does not start with 'aes:'", () => {
      expect(isAesString("$v=1$alg=A256GCM$data$")).toBe(false);
    });

    test("should return false for empty string", () => {
      expect(isAesString("")).toBe(false);
    });

    test("should return false for non-string input", () => {
      expect(isAesString(null as any)).toBe(false);
      expect(isAesString(undefined as any)).toBe(false);
      expect(isAesString(123 as any)).toBe(false);
      expect(isAesString({} as any)).toBe(false);
    });

    test("should return false for base64url-encoded string (no aes: prefix)", () => {
      expect(isAesString("eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIn0")).toBe(false);
    });
  });
});
