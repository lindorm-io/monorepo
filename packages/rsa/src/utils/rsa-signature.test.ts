...
describe("rsa-signature", () => {
  ...
  describe("verify", () => {
    test("should verify signature", () => {
      const signature = createRsaSignature({ key: PRIVATE_KEY, data });

      expect(verifyRsaSignature({ key: PUBLIC_KEY, data, signature })).toBe(true);
    });
  });
  ...
  describe("assert", () => {
    test("should assert signature", () => {
      ...
      expect(() => assertRsaSignature({ key: PUBLIC_KEY, data, signature })).not.toThrow();
    });
