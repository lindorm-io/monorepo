...
import { decryptRsaCipher, encryptRsaCipher } from "./rsa-cipher";
...
describe("rsa-cipher", () => {
  ...
  describe("encryption", () => {
    test("should create rsa cipher using private key", () => {
      expect(encryptRsaCipher({ data, key: PRIVATE_KEY })).toStrictEqual(expect.any(String));
    });

    test("should create rsa cipher using public key", () => {
      expect(encryptRsaCipher({ data, key: PUBLIC_KEY })).toStrictEqual(expect.any(String));
    });
  });

  describe("decryption", () => {
    test("should decrypt rsa cipher using private key", () => {
      const cipher = encryptRsaCipher({ data, key: PRIVATE_KEY });

      expect(decryptRsaCipher({ cipher, key: PUBLIC_KEY })).toBe(data);
    });

    test("should decrypt rsa cipher using public key", () => {
      const cipher = encryptRsaCipher({ data, key: PUBLIC_KEY });

      expect(decryptRsaCipher({ cipher, key: PRIVATE_KEY })).toBe(data);
    });
  });
});
