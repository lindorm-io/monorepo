...
describe("/internal/userinfo", () => {
  ...
  test("PUT /:id", async () => {
    ...
    expect(find(identifiers, { type: IdentifierTypes.EMAIL })).toStrictEqual(
      expect.objectContaining({
        identifier: email,
      }),
    );

    expect(find(identifiers, { type: IdentifierTypes.EXTERNAL })).toStrictEqual(
      expect.objectContaining({
        identifier: sub,
      }),
    );

    expect(find(identifiers, { type: IdentifierTypes.PHONE })).toStrictEqual(
      expect.objectContaining({
        identifier: phone,
      }),
    );
