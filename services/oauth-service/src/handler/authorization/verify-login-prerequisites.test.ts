...
describe("verifyLoginPrerequisites", () => {
  ...
  test("should return true", () => {
    expect(verifyLoginPrerequisites(ctx, authorizationRequest, browserSession)).toBe(true);
  });

  test("should return false when session is missing", () => {
    expect(verifyLoginPrerequisites(ctx, authorizationRequest)).toBe(false);
  });

  test("should return false on identity id returning false", () => {
    verifyIdentityId.mockReturnValue(false);

    expect(verifyLoginPrerequisites(ctx, authorizationRequest, browserSession)).toBe(false);
  });
