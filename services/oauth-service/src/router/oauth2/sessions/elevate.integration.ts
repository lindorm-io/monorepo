  ...
  test("should resolve initialised elevation with request body for refresh session", async () => {
    ...
    const response = await request(server.callback())
      ...
      .send({
        ...
        authentication_methods: ["bank_id_se"],
        client_id: client.id,
        country: "se",
    ...
    await expect(
      ...
      expect.objectContaining({
        ...
        nonce: "QxEQ4H21R-gslTwr",
        requestedAuthentication: {
          authenticationMethods: ["bank_id_se"],
          levelHint: 0,
          levelOfAssurance: 4,
          methodHint: [],
          missingAccessLevel: 0,
  ...
  test("should resolve verified elevation session", async () => {
    ...
    const elevationSession = await TEST_ELEVATION_SESSION_CACHE.create(
      createTestElevationSession({
        ...
        requestedAuthentication: {
          authenticationMethods: [AuthenticationMethod.BANK_ID_SE],
          levelHint: 2,
          levelOfAssurance: 4,
          methodHint: [AuthenticationMethod.PASSWORD],
