...
export const getTestAccessToken = (options: Partial<IssuerSignOptions<any, any>> = {}): string => {
  const { token } = getTestJwt().sign({
    ...
    permissions: [
      ...
      IdentityPermission.CLIENT_ADMIN,
      IdentityPermission.CLIENT_WRITE,
      IdentityPermission.CLIENT_READ,
      IdentityPermission.TENANT_ADMIN,
...
export const getTestClientCredentials = (
  ...
  const { token } = getTestJwt().sign({
    ...
    permissions: [
      ...
      ClientPermission.AUTHENTICATION_PUBLIC,
      ClientPermission.COMMUNICATION_CONFIDENTIAL,
      ClientPermission.COMMUNICATION_PUBLIC,
      ClientPermission.DEVICE_CONFIDENTIAL,
      ClientPermission.DEVICE_PUBLIC,
      ClientPermission.IDENTITY_CONFIDENTIAL,
      ClientPermission.IDENTITY_PUBLIC,
      ClientPermission.OAUTH_CONFIDENTIAL,
      ClientPermission.OAUTH_PUBLIC,
    ],
    scopes: [
