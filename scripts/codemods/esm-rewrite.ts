...
const isJestMockCall = (node: ts.CallExpression): boolean => {
  const expr = node.expression;
  if (!ts.isPropertyAccessExpression(expr)) return false;
  if (!ts.isIdentifier(expr.expression) || expr.expression.text !== "jest") return false;
  if (!ts.isIdentifier(expr.name)) return false;
  return expr.name.text === "mock" || expr.name.text === "unstable_mockModule";
};
        ...
        if (arg) {
          if (ts.isStringLiteral(arg) || ts.isNoSubstitutionTemplateLiteral(arg)) {
            ...
            warnings.push({
              ...
              line: lineOf(sourceFile, arg),
              message: `Non-literal jest.mock module argument left untouched`,
            });
          }
        }
